...
module.exports = async ({ width, height, data }) => {
    ...
    await page.addScriptTag({ path : Path.resolve(__dirname, './node_modules/chart.js/dist/chart.min.js') });
    await page.addScriptTag({ path : Path.resolve(__dirname, './node_modules/luxon/build/global/luxon.min.js') });
    await page.addScriptTag({ path : Path.resolve(__dirname, './node_modules/chartjs-adapter-luxon/dist/chartjs-adapter-luxon.js') });
    ...
    await FS.writeFile('../output/v3/puppeteer.png', image);
